 ...
 */
export async function contextGetContextRestrictions(
    client$: CircleciCore,
    request: operations.GetContextRestrictionsRequest,
    options?: RequestOptions
): Promise<
    Result<
        operations.GetContextRestrictionsResponse,
        | SDKError
        | SDKValidationError
        | UnexpectedClientError
        | InvalidRequestError
...
> {
    ...
    const parsed$ = schemas$.safeParse(
        input$,
        (value$) => operations.GetContextRestrictionsRequest$outboundSchema.parse(value$),
    ...
    >(
        ...
        m$.json([400, 401, 404, 429, 500], operations.GetContextRestrictionsResponse$inboundSchema)
    )(response);
    if (!result$.ok) {
        return result$;
    }
