 ...
 */
export async function policyManagementGetPolicyBundle(
    client$: CircleciCore,
    request: operations.GetPolicyBundleRequest,
    options?: RequestOptions
): Promise<
    Result<
        operations.GetPolicyBundleResponse,
        | SDKError
        | SDKValidationError
        | UnexpectedClientError
        | InvalidRequestError
...
> {
    ...
    const parsed$ = schemas$.safeParse(
        input$,
        (value$) => operations.GetPolicyBundleRequest$outboundSchema.parse(value$),
    ...
    >(
        ...
        m$.json([400, 401, 403, 500], operations.GetPolicyBundleResponse$inboundSchema)
    )(response);
    if (!result$.ok) {
        return result$;
    }
