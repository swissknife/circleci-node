 ...
 */
export async function projectDeleteCheckoutKey(
  client: CircleciCore,
  request: operations.DeleteCheckoutKeyRequest,
  options?: RequestOptions,
): Promise<
  Result<
    operations.DeleteCheckoutKeyResponse,
    | SDKError
    | SDKValidationError
    | UnexpectedClientError
    | InvalidRequestError
...
> {
  ...
  const path = pathToFunc("/project/{project-slug}/checkout-key/{fingerprint}")(
    pathParams,
  );
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, operations.DeleteCheckoutKeyResponse$inboundSchema),
    M.json("default", operations.DeleteCheckoutKeyResponse$inboundSchema),
  )(response);
  if (!result.ok) {
    return result;
  }
